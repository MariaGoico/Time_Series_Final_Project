...
def prepare_darts_from_split(X_train, y_train, X_test, y_test):
    ...
    DL_IGNORE = {
        ...
        'temp_lag_24', 'lag_price_24', 'lag_price_error_24'
    }

    FUTURE_KEYWORDS = ('holiday', 'bridge', 'next_day', '_sin', '_cos', 'sunrise', 'sunset', 'day_length', 'daylight')
    FUTURE_EXACT = {'day_of_week'}
    ...
    feature_cols    = [c for c in X_train.columns if c not in DL_IGNORE]
    future_cov_cols = [c for c in feature_cols if is_future(c)]
    past_cov_cols   = [c for c in feature_cols if not is_future(c)]

    print(f"  Future covariates : {len(future_cov_cols)} columns (Calendario + Solar)")
    print(f"  Past covariates   : {len(past_cov_cols)} columns (Clima real)")
    ...
    return {
        'train_target_scaled': train_target_scaled, 'test_target_scaled': test_target_scaled,
        'train_past_scaled': train_past_scaled, 'test_past_scaled': test_past_scaled,
        'train_future_scaled': train_future_scaled, 'test_future_scaled': test_future_scaled,
        'test_target_unscaled': test_target, 'scaler_target': scaler_target,
    }
...
def train_and_evaluate_deep_learning_fleet(
    ...
    full_past   = concatenate([train_past, test_past], axis=0) if train_past is not None else None
    full_future = concatenate([train_future, test_future], axis=0) if train_future is not None else None
    full_series = concatenate([train_tgt, test_tgt], axis=0)

    loggers = {}
    trainer_kwargs = {}
    for name, pat in [('LSTM',5),('GRU',5),('TCN',5),('N-BEATS',5),('N-HiTS',5)]:
    ...
    for name, config in models_dict.items():
        ...
        tr_past, vl_past = train_past.split_before(VAL_SPLIT) if train_past is not None else (None, None)
        tr_fut,  vl_fut  = train_future.split_before(VAL_SPLIT) if train_future is not None else (None, None)
